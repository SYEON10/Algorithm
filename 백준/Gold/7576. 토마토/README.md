...
코드는 딱딱하게, 사고는 유연하게.
늘 생각하는 것이지만 코드가 더러워지면 틀린다. 깔끔하게! 

✨토마토가 언제 익었는지 기록하여 날짜가 얼마나 지났는지 알 수 있다.
토마토 이차원배열에 day를 기록 -> 해당 토마토로 인해 익은 토마토 = day + 1

1. 상하좌우처럼 const한 값은 만들어놓고 사용하기
```
    int dr[] = {-1, 0, 0, 1};
    int dc[] = {0, -1, 1, 0};
```

2. auto 를 다음과 같이 사용할 수도 있다.
```
    auto[r, c] = BFS.front(); //BFS는 queue<pair<int, int>>
```
