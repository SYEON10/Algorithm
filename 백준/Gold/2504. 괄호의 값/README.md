...
내 풀이는 O(n)의 공간복잡도를 추가로 가져야 한다.
하지만 분배법칙을 이용하면 추가적인 공간을 사용하지 않고도 문제를 해결할 수 있다.

여는 괄호에서 값을 곱하고, 닫는 괄호에서 다시 값을 나누면 된다.
아직 스택이 주는 특징을 잘 활용하지 못하는 것 같다.
(넣을 때 처리하고 나올 때 역으로 처리하면 된다. )

### 제출 일자

2024년 1월 24일 11:28:06
